...
strings. For failed AI analyses, an audit may include only safe error metadata:
the final error type, attempt count, HTTP status, provider error code, and
request ID when available; it never includes a response body, error message, or
credential. With `pre_analysis_title_dedup_enabled`, Horizon also collapses only
